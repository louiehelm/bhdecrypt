case button_main_process
	if msg.message=wm_lbuttondown then
		...
		if ui_listbox_gettext(list_main,ui_listbox_getcursel(list_main))="Substitution + crib grid" andalso wc_windowup=1 then soi="Ok" 'exception
		if soi="Ok" then 'if cipher is ok		
				...
				if task_active="none" then
					...
					if solverexist=1 then
						select case ui_listbox_gettext(list_main,ui_listbox_getcursel(list_main))
							case "Substitution","Substitution + sequential homophones"
								thread_ptr(threadsmax+1)=threadcreate(@thread_solve_substitution,0)
							case "Substitution + all-grams"
								thread_ptr(threadsmax+1)=threadcreate(@thread_solve_allgrams,0)
							case "Substitution + crib grid","Bigram substitution + crib grid"
								...
								select case ui_listbox_gettext(list_main,ui_listbox_getcursel(list_main))
									...
									case "Bigram substitution + crib grid":solvesub_cribgridinstance=1
								end select
								if wc_windowup=0 or prev_solvesub_cribgridinstance<>solvesub_cribgridinstance then
									dim as long wc_x0,wc_y0,wc_x1,wc_y1
									ui_window_getposition(window_main,wc_x0,wc_y0,wc_x1,wc_y1)
							...
							case "Substitution + sparse polyalphabetism"		
								thread_ptr(threadsmax+1)=threadcreate(@thread_solve_sparsepolyalphabetism,0)		
