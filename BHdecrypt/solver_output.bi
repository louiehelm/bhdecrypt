	...
	if improved=1 then
		if local_outputbatch=0 then
			...
			if solvesub_overwriteoutput=0 andalso fileexists(filename) then
				...
				solstring="Score: "+format(thread(tn).score,"0.00")+" IOC: "+format(thread(tn).ioc,"0.0000")+" Multiplicity: "+format(thread(tn).multiplicity,"0.0000")+stt(thread(tn).sectime)+lb
				...
				select case solver_output
					...
					case 2:solstring+="Average n-gram size: "+format(thread(tn).tmpd1,"0.0000")+lb+lb
					case 3:solstring+="Polyalphabetism: "+format((1-thread(tn).match)*100,"0.00")+"%"+lb+lb
			...
			solstring+=", Score: "+format(thread(tn).score,"0.00")
			solstring+=" IOC: "+format(thread(tn).ioc,"0.0000")
			solstring+=" Multiplicity: "+format(thread(tn).multiplicity,"0.0000")
			...
			select case solver_output
				...
				case 2:solstring+=" Average n-gram size: "+format(thread(tn).tmpd1,"0.0000")
				case 3:solstring+=" Polyalphabetism: "+format((1-thread(tn).match)*100,"0.00")+"%"
