z=0  'TODO: fix this optimization that used to work
'if ((in_picker = 0 OrElse m=5) AndAlso ngrams(j-1) > 0) OrElse _  ' skip most checks if previous ngram passed them
 if  g5keyguard(sol(j),sol(j+1),sol(j+2),sol(j+3),sol(j+4)) > 0 Then

	If g4keyguard(sol(j+2),sol(j+3),sol(j+4),sol(j+5)) > 0 Then
