...
load_status+="N-gram factor: "+rdc(solvesub_ngramfactor,5)+lb
...
if ngram_size=8 andalso solvesub_ngramcaching=1 then
	...
	load_status+="N-gram caching memory usage: "+rdc(cachebh8_nfb/1073741824,2)+" GB RAM ("+str(cachebh8_nfb)+" bytes)"+lb
end if
