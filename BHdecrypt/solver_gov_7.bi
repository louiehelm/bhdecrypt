z=0
If (in_picker = 0 OrElse m=6) AndAlso ngrams(j-1) > 0 Then
	If g4keyguard(sol(j+3),sol(j+4),sol(j+5),sol(j+6)) > 0 Then  ' skip most checks if previous ngram passed them
