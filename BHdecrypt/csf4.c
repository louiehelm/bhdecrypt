...
int64_t csf4_get_byte_array_fb(const uint64_t *gov_array, const uint64_t *gov_offset_and_seed, const uint64_t gov_multiplier, const uint64_t gov_globalseed, const int w, const uint64_t end, const uint64_t start, const uint64_t *gov_symbol, const uint64_t *gov_last_codeword_plus_one, const uint32_t *gov_how_many_up_to_block, const uint8_t *gov_shift, const char *key, const uint64_t length) {
// int64_t csf4_get_byte_array_fb(const uint64_t *gov_array, const uint64_t *gov_offset_and_seed, const uint64_t gov_multiplier, const uint64_t gov_globalseed, const uint64_t gov_global_max_codeword_length, const uint64_t gov_escaped_symbol_length, const uint64_t gov_escape_length, const uint64_t *gov_symbol, const uint64_t *gov_last_codeword_plus_one, const uint32_t *gov_how_many_up_to_block, const uint8_t *gov_shift, const char *key, const uint64_t length) {
...
//	const int w = gov_global_max_codeword_length;
	...
	const int64_t t = decode2(gov_symbol, gov_last_codeword_plus_one, gov_how_many_up_to_block, gov_shift, get_value(gov_array, e[0] + bucket_offset, w) ^ get_value(gov_array, e[1] + bucket_offset, w) ^ get_value(gov_array, e[2] + bucket_offset, w) ^ get_value(gov_array, e[3] + bucket_offset, w));
	if (t != -1) return t;
//	const uint64_t end = gov_global_max_codeword_length - gov_escape_length;
//	const uint64_t start = end - gov_escaped_symbol_length;
	return get_value(gov_array, e[0] + start, e[0] + end) ^ get_value(gov_array, e[1] + start, e[1] + end) ^ get_value(gov_array, e[2] + start, e[2] + end) ^ get_value(gov_array, e[3] + start, e[3] + end);
}