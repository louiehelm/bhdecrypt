...
int64_t sf3_get_byte_array_fb(const uint64_t *gov_array, const uint64_t *gov_offset_and_seed, uint64_t gov_width, uint64_t gov_multiplier, uint64_t gov_globalseed, char *key, uint64_t length) {
	...
	return get_value(gov_array, e[0] + bucket_offset, gov_width) ^ get_value(gov_array, e[1] + bucket_offset, gov_width) ^ get_value(gov_array, e[2] + bucket_offset, gov_width);
}
